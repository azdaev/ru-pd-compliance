...
Ответь на вопросы по порядку и собери свой набор обязательных действий.

### 1. У тебя есть сайт?

```
Да → читай дальше
Нет → этот гайд тебе пока не нужен
```

### 2. Ты собираешь хоть какие-то данные? (имя, телефон, email, формы заявок)

```
Да → тебе нужно:
  ✅ Политика конфиденциальности (в футер)
  ✅ Согласие на обработку ПД (в футер, отдельный документ)
  ✅ Чекбоксы в формах (не предзаполненные, с кликабельными ссылками)
  ✅ Регистрация в Роскомнадзоре
  ✅ Сервер в России
  ✅ Процесс ответа на запросы пользователей (10 дней)

Нет → переходи к вопросу 3
```

### 3. На сайте стоит аналитика? (Яндекс.Метрика, счетчики, рекламные пиксели)

```
Да → тебе нужно:
  ✅ Соглашение об использовании куки (отдельный документ или раздел в политике)
  ✅ Куки-баннер (плашка с "Принять" / "Отказаться")

Нет → куки-документы не нужны
```

### 4. Ты продаешь что-то через сайт? (товары, услуги, курсы, подписки)

```
Да → тебе дополнительно нужно:
  ✅ Публичная оферта (договор купли-продажи, в футер)
  ✅ Чекбокс ссылается на оферту вместо согласия

Нет → оферта не нужна, хватит политики + согласия
```

### 5. На сайте есть регистрация/вход?

```
Да → авторизация только через российские сервисы:
  ✅ Телефон, Госуслуги, VK, Mail.ru
  ❌ Убрать Google/Apple Sign-In

Нет → этот пункт не касается тебя
```

### 6. Используешь зарубежные сервисы, которые собирают данные?

```
Google Analytics, Google Forms, Google reCAPTCHA,
зарубежный хостинг, Supabase (оригинальный), Lovable Cloud

Да → убрать ВСЕ до регистрации в Роскомнадзоре:
  ✅ Google Analytics → Яндекс.Метрика
  ✅ Google reCAPTCHA → российские аналоги
  ✅ Зарубежный хостинг → российский сервер
  ⚠️ Google Search Console можно оставить (не собирает ПД)

Нет → ты молодец
```

### Итого — быстрая сводка

| У тебя на сайте | Что нужно |
|-----------------|-----------|
| Просто сайт без форм, но с аналитикой | Политика + куки-баннер + Роскомнадзор |
| Формы заявок (имя, телефон, email) | Политика + согласие + чекбоксы + куки-баннер + Роскомнадзор |
| Интернет-магазин / продажа услуг | Всё выше + публичная оферта |
| Регистрация / личный кабинет | Всё выше + только российская авторизация |
